...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if self.is_container:
            if self.ant_contained and self.ant_contained.double_done:
                self.ant_contained.damage *= 2
                self.ant_contained.double_done = False
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            if current_place.bees == [] or not self.min_range <= distance <= self.max_range:
                distance += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        copy_bees = ant_place.bees[:]
        Ant.reduce_health(self, amount)
        reflected_damage = amount
        if self.health <= 0:
            reflected_damage = self.damage + amount
        for bee in copy_bees:
            Ant.reduce_health(bee, reflected_damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown == 0:
            if self.place.bees:
                random_b = random_bee(self.place.bees)
                random_b.reduce_health(random_b.health)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if gamestate.is_constructed:
            return None
        else:
            gamestate.is_constructed = True
    ...
    def action(self, gamestate):
        ...
        while current_place.exit:
            ant_in_place = current_place.exit.ant
            if ant_in_place:
                ant_in_place.buff()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if gamestate.time % 2 == 1 and self.slow_time:
                pass
